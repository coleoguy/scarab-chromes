...
all_luc <- read.csv('../results/lucanidae.all.csv')
all_pass <- read.csv('../results/passalidae.all.csv')
all_sca <- read.csv('../results/scarbeidae.all.csv')
plot(all_luc$p[1:100], type = 'l', ylim = c(-60,-40), main = 'Lucanidae', ylab = '')
...
plot(all_pass$p[1:100], type = 'l', ylim = c(-120, -65), main = 'Passalidae', ylab = '')
...
plot(all_sca$p[1:100], type = 'l', ylim = c(-205,-165), main = 'Scarabeidae', ylab = '')
...
dat_luc <- read.csv('../results/lucanidae.mod1.csv')
dat_pass <- read.csv('../results/passalidae.mod1.csv')
dat_sca <- read.csv('../results/scarbeidae.mod1.csv')

# asc
# scarab
cols <- viridis(3, option = 'D',alpha = 0.7, begin = 0.45)
plot(density((dat_sca$asc1)),main ='',xlab='Fission (/MY)',
     ylim =c(-0.3,8), xlim=c(0,8.5),)
polygon(density(dat_sca$asc1),col=cols[1])
hpd <- HPDinterval(as.mcmc(dat_sca$asc))
y <- -.1
lines(y=c(y,y), x=hpd[1:2], lwd=2,col=cols[1])
# lucanidae
lines(density(dat_luc$asc1))
polygon(density(dat_luc$asc1),col=cols[2])
hpd <- HPDinterval(as.mcmc(dat_luc$asc))
y <- -.2
lines(y=c(y,y), x=hpd[1:2], lwd=2,col=cols[2])
# passalidae
lines(density((dat_pass$asc1)))
polygon(density(dat_pass$asc1),col=cols[3])
hpd <- HPDinterval(as.mcmc(dat_pass$asc))
y<- -.3
...
#desc
cols <- viridis(3, option = 'D',alpha = 0.7, begin = 0.45)
plot(density(dat_sca$desc1),main ='',xlab='Fusion (/MY)',xlim= c(0,10), ylim =c(-0.02,4))
polygon(density(dat_sca$desc1),col=cols[1])
hpd <- HPDinterval(as.mcmc(dat_sca$desc))
lines(y=c(-0.03,-0.03), x=hpd[1:2], lwd=2,col=cols[1])
lines(density((dat_luc$desc1)))
polygon(density(dat_luc$desc1),col=cols[2])
hpd <- HPDinterval(as.mcmc(dat_luc$desc))
lines(y=c(-0.07,-0.07), x=hpd[1:2], lwd=2,col=cols[2])
lines(density((dat_pass$desc1)))
polygon(density(dat_pass$desc1),col=cols[3])
hpd <- HPDinterval(as.mcmc(dat_pass$desc))
...
tree <- read.tree('../data/6.BEAST/result_1000000000/finaltree')[[37]]
...
chrom.s$gen.prob <- 1
for(i in 1:nrow(chrom)){
  if(chrom.s$SCS[i] %in% c('XY','Xyp','Xyr','NeoXY','Xy')){
...
chrom.mat_ <- datatoMatrix(x = chrom.s[,c(1,2,4)],
                           range = rng,
                           hyper = F)
results <- readRDS('../results/simple_model_number.rds')
